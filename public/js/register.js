  ...
  if (form) {
    form.addEventListener("input", (e) => {
      ...
      if (name === "firstN" || name === "add") {
        ...
        const label = name === "firstN" ? "Full name" : "Address";

        if (name === "firstN") {
          if (!/^[A-Za-z\s]+$/.test(value)) {
            err.innerText = `${label} must contain only letters.`;
            return;
          }
        }
        ...
        if (name === "add") {
          if (/^[0-9-,]/.test(value)) {
            err.innerText = `${label} cannot start with number, '-' or ','.`;
            return;
          }

          if (!/^[A-Za-z0-9\s,-]+$/.test(value)) {
            err.innerText = `${label} contains invalid characters.`;
            return;
          }

          const digitCount = (value.match(/\d/g) || []).length;
          if (digitCount > 2) {
            err.innerText = `${label} can contain at most 2 numbers.`;
            return;
          }
        }
      ...
      if (name == "address") {
        ...
        if (value.length === 0) {
          ...
          err.innerText =
            "Must start with letters or only - and , are allowed!";
        } else if (value.length > 25) {
          err.innerText = "Letters must be less than 25!";
        } else {
          err.innerText = "";
